...
class TravelClaimController extends Controller
{
    public function __construct()
    {
        $this->middleware(function ($request, $next) {
            ...
            // Allow Admins, HR, and Accounts for processing
            if ($user->hasRole(['admin', 'Admin', 'super_admin', 'hr', 'HR', 'accounts', 'Accounts', 'manager', 'Manager'])) {
                return $next($request);
            }

            // For regular employees, strictly allow only Sales Department
            if ($user->department !== 'Sales Department') {
                abort(403, 'The Travel & Expense module is strictly restricted to Sales Department employees.');
            }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'items.*.mode_of_travel' => 'required|string',
...
public function update(Request $request, $id)
{
    ...
    public function pay(Request $request, $id)
    {
        ...
        if ($request->payment_type == '85') {
            $claim->update(['split_85_transaction' => $request->transaction]);
        } elseif ($request->payment_type == '15') {
            $claim->update(['split_15_transaction' => $request->transaction]);
        }
    ...
    {
        $query = TravelClaim::with('user')->orderBy('created_at', 'desc');
