...
class JobController extends Controller
{
    ...
    public function store(Request $request)
    {
        if (Auth::user()->can('Create Job')) {
            ...
            $job->description          = strip_tags($request->description, '<b><i><u><a><p><br><ul><li><ol><h1><h2><h3><h4><h5><h6>');
            $job->requirement          = strip_tags($request->requirement, '<b><i><u><a><p><br><ul><li><ol><h1><h2><h3><h4><h5><h6>');
            $job->terms_and_conditions = !empty($request->terms_and_conditions) ? strip_tags($request->terms_and_conditions, '<b><i><u><a><p><br><ul><li><ol><h1><h2><h3><h4><h5><h6>') : '';
            $job->code                 = uniqid();
    ...
    public function career($lang = 'en')
    {
        $query = Job::where('status', 'active')->with(['createdBy', 'branches', 'categories']);
        ...
        return view('tenant.job.career', compact('companySettings', 'jobs', 'categories', 'locations', 'languages', 'currantLang', 'id'));
    }
    ...
    {
        try {
            $job = Job::where('id', $id)->where('status', 'active')->firstOrFail();
        } catch (\Throwable $th) {
            return redirect()->back()->with('error', 'Page not Found');
        }
