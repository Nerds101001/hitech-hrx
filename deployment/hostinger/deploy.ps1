param(
  ...
  [string]$DbPassword  = "",
  [switch]$SkipBuild   # Pass -SkipBuild to skip npm run build (e.g. PHP-only changes)
)
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 0: Build frontend assets locally (always, unless -SkipBuild is passed)
# ─────────────────────────────────────────────────────────────────────────────
if ($SkipBuild) {
  Write-Host "[0/8] Skipping npm build (-SkipBuild flag set)."
} else {
  Write-Host "[0/8] Building frontend assets (npm run build)..."
  $npmResult = & npm run build 2>&1
  if ($LASTEXITCODE -ne 0) {
    Write-Host $npmResult
    throw "npm run build FAILED. Fix build errors before deploying."
  }
  Write-Host "      Build successful. public/build/ is up to date."
}
...
Write-Host "[1/8] Building release archive (including public/build/)..."
tar -czf $archive --exclude-from=deployment/hostinger/exclude.lst .

Write-Host "[2/8] Uploading release archive and deploy script..."
...
Write-Host "[3/8] Running remote deployment..."
...
if ($ImportDatabase) {
  if (-not $SqlFile) {
    Write-Host "[4/8] SQL file not provided. Exporting from local .env database..."
  ...
  Write-Host "[5/8] Uploading SQL dump..."
  ...
  Write-Host "[6/8] Backing up production DB and importing new data..."
  $dbCmd = @"
...
Write-Host "[8/8] Done. Deployment completed successfully."
Write-Host "      Built assets: public/build/ | Deployed to: $AppRoot"
